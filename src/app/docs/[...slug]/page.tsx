...
import { ProgressBar } from "@/components/note/progress-bar";
...
export default async function NotePage({
  ...
  return (
    <>
      ...
      >
        <article className="animate-fade-up px-4 py-8 sm:px-6 sm:py-12" style={{ animationDelay: "100ms" }}>
          <header className="mb-12 border-b border-[var(--border-soft)] pb-8">
            ...
            <h1 className="mt-6 font-serif text-5xl leading-[1.05] tracking-tight text-[var(--foreground)] sm:text-6xl">
              {note.title}
            </h1>
            <p className="mt-6 max-w-[85%] text-lg leading-relaxed text-[var(--muted)]">
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                {note.tags.map((tag) => (
                  <Link
                    ...
                    className="rounded-full border border-[var(--border-strong)] bg-[var(--surface-elevated)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.1em] text-[var(--muted)] transition-all duration-[--motion-fast] hover:bg-[var(--surface)] hover:text-[var(--foreground)]"
                  >
                    #{tag}
                  </Link>
                ))}
